...
class EliminacaoGauss:
    ...
    def __init__(self, matrizA, matrizb):
        ...
        if self.nLinhasA == self.nColA:
            matrizA_equivalente, vetorb_equivalente = self._eliminar()
            resultado = self._substituir(matrizA_equivalente, vetorb_equivalente)
            print(resultado)
        else:
    ...
    def _substituir(self, matriz_triangular_superior, matrizb):
        x = self.x
        n = self.nLinhasb
